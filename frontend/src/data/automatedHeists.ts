...
export const automatedHeists: AutomatedHeist[] = [
  ...
  // ===== BEGINNER TIER =====
  {
    ...
    riskLevel: 2,
    description:
      'Quick in-and-out job at a local convenience store. Perfect for beginners to learn the ropes.',
    unlockConditions: [],
  },
  ...
  // ===== AMATEUR TIER =====
  {
    ...
    riskLevel: 3,
    description:
      'Target wealthy apartments when residents are away. Requires careful timing and stealth.',
    unlockConditions: [],
  },

  {
    ...
    riskLevel: 4,
    description:
      'Hit an electronics store for valuable gadgets. Alarm systems make this challenging.',
  ...
  // ===== EXPERT TIER =====
  {
    ...
    riskLevel: 8,
    description:
      'The classic bank job. Requires a full professional crew with specialized equipment.',
    unlockConditions: [
      'Complete 20 professional heists',
      'Reputation 2000+',
      'Team average level 6+',
    ],
  },
  ...
  // ===== SPECIAL EVENT HEISTS =====
  {
    ...
    riskLevel: 5,
    description:
      'Infiltrate a high-society museum gala to steal priceless artifacts. Requires social finesse.',
...
export function isHeistAvailable(heist: AutomatedHeist, playerState: GameState): boolean {
  ...
  for (const condition of heist.unlockConditions || []) {
    ...
    if (condition.includes('Have') && condition.includes('character')) {
      const requiredClass = condition.match(
        /(Face|Tech|Muscle|Infiltrator|Acrobat|Mastermind|Wildcard)/
      )?.[0];
      if (requiredClass) {
...
export function calculateHeistOutcome(
  heist: AutomatedHeist,
  team: TeamMember[]
): {
  success: boolean;
