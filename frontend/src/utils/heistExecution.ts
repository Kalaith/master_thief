...
export function resolveEncounter(
  ...
  if (encounter.primary_attribute) {
    ...
    switch (encounter.primary_skill) {
      ...
      case 'athletics':
        attributeModifier =
          getAttributeModifier(member.attributes.strength) +
      ...
      case 'social':
        attributeModifier =
          getAttributeModifier(member.attributes.charisma) +
          getAttributeModifier(member.attributes.wisdom);
        break;
      default:
        attributeModifier = 0;
...
function generateNarrativeDescription(
  ...
  if (Math.random() < 0.3) {
    // 30% chance for special ability reference
  ...
  team.forEach(member => {
    Object.keys(totalSkills).forEach(skill => {
      totalSkills[skill as keyof typeof totalSkills] +=
        member.skills[skill as keyof typeof member.skills];
    });
  });
...
export function simulateAutomatedHeist(
  ...
  const successfulEncounters = results.filter(
