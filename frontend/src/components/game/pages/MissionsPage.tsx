...
const MissionsPage: React.FC<MissionsPageProps> = ({ onBackToGame }) => {
  ...
  const handleStartHeist = (heist: AutomatedHeist) => {
    if (canStartHeist(heist)) {
      ...
      if (selectedTeam.length < heist.requirements.minTeamSize) {
        toast.error(
          `You need at least ${heist.requirements.minTeamSize} team members to start this mission.`
        );
      } else if (selectedTeam.length > heist.requirements.maxTeamSize) {
        toast.error(
          `You can only have ${heist.requirements.maxTeamSize} team members for this mission.`
        );
      } else {
  ...
  const handleTeamAssignmentConfirm = () => {
    if (missionForTeamAssignment) {
      ...
      toast.success(
        `Mission "${missionForTeamAssignment.name}" started! Check the Active Missions section to monitor progress.`
      );
            ...
            {displayHeists.length === 0 ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {displayHeists.map(heist => (
                  <div
                    key={heist.id}
                    className="bg-heist-panel border border-heist-border rounded-xl p-4 hover:border-cyan-400/50 hover:shadow-cyan-glow transition-all"
                  >
            ...
            {activeAutomatedHeists.length === 0 ? (
              <div className="bg-heist-panel border border-heist-border rounded-xl p-12 text-center">
                ...
                <p className="text-gray-500 font-mono text-sm">
                  Select a target above to deploy your crew.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {activeAutomatedHeists.map((activeHeist, index) => (
                  <div
                    key={index}
                    className="bg-heist-panel border border-cyan-400/30 rounded-xl p-6 shadow-hud-panel"
                  >
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      <div className="bg-heist-dark/60 border border-heist-border rounded p-3">
                        ...
                        <div
                          className={`font-bold font-mono ${
                            activeHeist.heist.riskLevel <= 3
                              ? 'text-emerald-400'
                              : activeHeist.heist.riskLevel <= 6
                                ? 'text-amber-400'
                                : 'text-red-400'
                          }`}
                        >
