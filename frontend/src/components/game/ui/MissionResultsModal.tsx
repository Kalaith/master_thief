...
import { CheckCircle, XCircle, Clock, Target, DollarSign, Star, TrendingUp, Users, AlertTriangle, Gift } from 'lucide-react';
            ...
            <div className="bg-heist-dark/60 border border-heist-border rounded-xl p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                <div className="bg-heist-panel border border-heist-border rounded p-3">
                  ...
                  <div className="text-cyan-400 font-mono font-bold">{formatDuration(heist.duration)}</div>
                </div>
                <div className="bg-heist-panel border border-heist-border rounded p-3">
                  <div className="text-gray-400 font-mono uppercase text-xs mb-1">Risk Level</div>
                ...
                <div className="bg-heist-panel border border-heist-border rounded p-3">
                  <div className="text-gray-400 font-mono uppercase text-xs mb-1">Team Power</div>
                  <div className="text-purple-400 font-mono font-bold">{teamPower.toFixed(0)}</div>
                </div>
                <div className="bg-heist-panel border border-heist-border rounded p-3">
                  <div className="text-gray-400 font-mono uppercase text-xs mb-1">Success Rate</div>
                  <div className={`font-mono font-bold ${
            ...
            <div className="bg-heist-dark/60 border border-heist-border rounded-xl p-4">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center p-3 bg-heist-panel border border-amber-400/30 rounded">
                  <span className="text-gray-300 font-mono uppercase text-sm">Cash Earned</span>
                  <span className="text-2xl font-bold font-mono text-amber-300">${rewards.payout}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-heist-panel border border-cyan-400/30 rounded">
                  ...
                  <span className="text-2xl font-bold font-mono text-cyan-400">{rewards.totalXP} XP</span>
                </div>
                {rewards.reputation > 0 && (
                  <div className="flex justify-between items-center p-3 bg-heist-panel border border-purple-400/30 rounded">
                    <span className="text-gray-300 font-mono uppercase text-sm">Reputation Gained</span>
                    <span className="text-2xl font-bold font-mono text-purple-400">+{rewards.reputation}</span>
                  </div>
                )}
                    ...
                    {rewards.equipmentDrops.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between mb-2 last:mb-0">
                        ...
                        <div className={`px-3 py-1 rounded text-xs font-bold font-mono border ${
                          item.rarity === 'legendary' ? 'bg-amber-400/20 border-amber-400 text-amber-400' :
                          item.rarity === 'masterwork' ? 'bg-purple-400/20 border-purple-400 text-purple-400' :
                          item.rarity === 'advanced' ? 'bg-blue-400/20 border-blue-400 text-blue-400' :
                          item.rarity === 'improved' ? 'bg-emerald-400/20 border-emerald-400 text-emerald-400' :
                    ...
                    <div key={member.id} className={`p-3 rounded border ${
                      ...
                      <div className="flex justify-between items-center">
                        <div className="flex-1">
                          ...
                          <div className="text-gray-400 text-sm font-mono">{member.specialty}</div>
                        </div>
                        <div className="text-right">
                          <div className="text-cyan-400 font-bold font-mono">+{rewards.experiencePerMember} XP</div>
                          <div className="text-gray-500 text-xs font-mono">Level {member.progression.level}</div>
                        </div>
                      </div>
                      {levelUp && (
                        <div className="mt-2 pt-2 border-t border-amber-400/30">
                          <div className="text-amber-400 text-sm font-mono uppercase">New Rewards:</div>
            ...
            {!success && (
              <div className="bg-red-500/20 border-2 border-red-400/50 rounded-xl p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-red-400 font-bold font-mono uppercase mb-2">Mission Failed</h4>
                    ...
                    <p className="text-gray-300 text-sm mt-2">
                      <strong className="text-red-400">Tip:</strong> Level up your team, acquire better equipment, or choose lower-risk missions.
                    </p>
                  </div>
                </div>
              </div>
